...
private:
  ...
  void _vector_to_bitmap(const uint16_t x, const uint16_t y, bitmap *bm,
                         const std::vector<bool> &character) const;

  // Inserts pxiels that forms the text in the line into the bitman
  bool _print(bitmap *bm, const uint16_t offset_y) const;
...
public:
  ...
  void offsetX(const uint16_t x) { _offset_x = x; };
  uint16_t offsetY() const { return _offset_y; };
  void offsetY(const uint16_t y) { _offset_y = y; };

  uint8_t fontSize() const { return _font_size; };
  void fontSize(const uint8_t fs) { _font_size = fs; };

  const color_t &color() const { return _color; };
  void color(const color_t &c) { _color = c; };
  ...
  const font_t &font() const { return _font; };

  uint8_t characterSpacing() const { return _character_spacing; };
  void characterSpacing(const uint8_t cs) { _character_spacing = cs; };
